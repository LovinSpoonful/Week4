...
DROP VIEW IF EXISTS vw_top_trades;

CREATE VIEW `vw_top_trades` AS
    SELECT 
        `i`.`item_name` AS `item_name`,
        `rb`.`region_name` AS `buy_region_name`,
        `tt`.`buy_kunits_avail` AS `buy_kunits_avail`,
        `tt`.`buy_avg_price` AS `buy_avg_price`,
        `rs`.`region_name` AS `sell_region_name`,
        `tt`.`sell_kunits_avail` AS `sell_kunits_avail`,
        `tt`.`sell_avg_price` AS `sell_avg_price`,
        `tt`.`unit_margin` AS `unit_margin`
    FROM
        (((`tbl_top_trades` `tt`
        JOIN `tbl_item` `i` ON ((`i`.`item_id` = `tt`.`item_id`)))
        JOIN `tbl_region` `rb` ON ((`rb`.`region_id` = `tt`.`buy_region_id`)))
        JOIN `tbl_region` `rs` ON ((`rs`.`region_id` = `tt`.`sell_region_id`)));
